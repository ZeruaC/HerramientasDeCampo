...
type TabId = 'instalacion' | 'problemas' | 'economico';
...
export const H1 = () => {
  ...
  const [activeTab, setActiveTab] = useState<TabId>('instalacion');

  const annualLoss = outagesPerYear * durationHours * costPerHour;
  const fiveYearLoss = annualLoss * 5;
  const economicComplete = outagesPerYear > 0 && durationHours > 0 && costPerHour > 0;
  ...
  const tabs: { id: TabId; label: string; icon: typeof Zap }[] = [
    { id: 'instalacion', label: 'Instalación Actual', icon: Zap },
    { id: 'problemas', label: 'Problemas Detectados', icon: AlertCircle },
    { id: 'economico', label: 'Impacto Económico', icon: DollarSign },
  ];

  return (
    <div className="p-4 md:p-8 max-w-6xl mx-auto">
      <div className="mb-6 md:mb-8">
        <h1 className="text-2xl md:text-3xl font-bold text-gray-800">H1 · Cuantificador del Dolor del Cliente</h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 space-y-4">
          ...
          <div className="bg-green-50 rounded-lg shadow-sm p-4 border border-green-200 flex items-center gap-6">
            <Building2 className="w-5 h-5 text-green-700 flex-shrink-0" />
            <div className="text-sm text-green-900 flex flex-wrap gap-x-6 gap-y-1">
              <span><strong>Cliente:</strong> {clientName || 'No especificado'}</span>
              <span><strong>Sector:</strong> {sector || 'No especificado'}</span>
            </div>
          </div>
            ...
            <div className="flex overflow-x-auto border-b border-gray-200 bg-gray-50">
              {tabs.map(({ id, label, icon: Icon }) => (
                ...
                >
                  ...
                  {label}
                  {id === 'economico' && !economicComplete && (
                    <span className="w-2 h-2 rounded-full bg-red-500" title="Requerido para H5" />
                  )}
                </button>
              ))}
            </div>
            ...
            <div className="p-4 md:p-6">
              ...
              {activeTab === 'economico' && (
                <div className="space-y-4">
                  <div className="p-4 bg-red-100 border-l-4 border-red-600 text-red-800 text-sm rounded-r-md">
        ...
        <div className="space-y-6">
          <div className="bg-red-50 rounded-lg shadow-md p-6 border border-red-100 lg:sticky lg:top-4">
